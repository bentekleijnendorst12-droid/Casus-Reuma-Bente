...
echtedata=read.table("count_matrix_RA.txt", header = TRUE, row.names = 1)
# colnames geven
colnames(echtedata) <- c("norm1", "norm2", "norm3", "norm41", "reuma1", "reuma2", "reuma3", "reuma4")
# metadata
Status=c ("Gezond", "Gezond", "Gezond", "Gezond", "Reuma", "Reuma", "Reuma", "Reuma")
Status_table= data.frame(Status)
#rijnamen
rownames(Status_table)=c("norm1", "norm2", "norm3", "norm41", "reuma1", "reuma2", "reuma3", "reuma4")
#desseq dataset
...
#Deg gefilterd op kleiner en gelijk aan -1 en groter of gelijk aan 1
DEG= ALLtabel %>%
  filter(padj<0.05)%>%
...
# 1. Selecteer de top 10 meest significante pathways en bereken -log10(p-waarde)
top10_GO <- GO.wall %>%
